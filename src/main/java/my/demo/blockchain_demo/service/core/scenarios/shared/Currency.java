...
public class Currency extends DynamicStruct {
    ...
    public Currency(String address, BigInteger minAmount, BigInteger maxAmount, boolean active, boolean enabled, List<String> approvedBy) {
        super(new Address(address), new Uint(minAmount), new Uint(maxAmount),
                new Bool(active), new Bool(enabled), new DynamicArray(approvedBy.stream().map(Address::new).toList()));
    ...
    public Currency(Address address, Uint minAmount, Uint maxAmount, Bool active, Bool enabled, DynamicArray<Address> approvedBy) {
